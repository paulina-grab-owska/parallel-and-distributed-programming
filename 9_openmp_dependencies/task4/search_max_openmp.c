                           ...
                           )
{
        ...
        if(level >= max_level){return search_max(A,p,r);}

        int s = (p +r) / 2;
        double a_max_left, a_max_right;

#pragma omp task shared(a_max_left) firstprivate(A, p, s, level)
        {
                a_max_left = bin_search_max_task(A, p, s, level +1);
        }


#pragma omp task shared(a_max_right) firstprivate(A, s, r, level)
        {
                a_max_right = bin_search_max_task(A, s+1, r, level +1);
        }

#pragma omp taskwait

        return (a_max_left > a_max_right) ? a_max_left : a_max_right;
